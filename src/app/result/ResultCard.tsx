...
type Props = { menus: string[]; who: string[]; groupName: string };

export default function ResultCard({ menus, who, groupName }: Props) {
  const router = useRouter();
  const menu = menus[0] || "오늘 메뉴";
  const icon = getFoodIconUrl(menu);
  ...
  return (
    <div style={{ padding: "28px 20px 40px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
      ...
      <div style={{
        ...
        {icon
          ? <img src={icon} alt={menu} style={{ width: "78%", height: "78%", objectFit: "contain" }} />
          : <span style={{ fontSize: 92 }}>🍽️</span>}
      </div>

      <p style={{ fontFamily: "var(--font-display)", fontSize: 32, margin: "0 0 4px" }}>{menu}</p>
      {menus.length > 1 && (
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, width: "100%", maxWidth: 320 }}>
        ...
        <button className="tap" onClick={async () => {
          const r = await shareResult({ menus, groupName: groupName || undefined, who });
