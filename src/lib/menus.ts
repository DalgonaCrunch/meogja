...
export const MENU_CATEGORIES: MenuCategory[] = [
  ...
  /* 브랜드명(뿌링클·황금올리브·교촌치킨)은 owner 가 일부러 넣은 것이다 — 빼지 않는다.
     사람들이 실제로 그 이름으로 시켜 먹는다. */
  { emoji:"🍗", label:"치킨", menus:["후라이드치킨","양념치킨","간장치킨","파닭","마늘치킨","순살치킨","핫윙","반반치킨","뿌링클","황금올리브","교촌치킨","닭발","닭강정","치즈닭갈비","찜닭","간장찜닭"] },
  ...
  /* `청양떡볶이`·`매운찜닭`·`매운낙지볶음` 처럼 "매운" 을 붙여 만든 이름만 뺐다.
     `매운족발`·`매운돼지갈비` 는 따로 파는 메뉴이고(owner 확인), 브랜드명(엽기떡볶이·
     불닭)은 일부러 넣은 것이다. */
  { emoji:"🌶️", label:"매운맛", menus:["김치찌개","순두부찌개","부대찌개","제육볶음","닭갈비","짬뽕","떡볶이","마라탕","마라샹궈","낙지볶음","쭈꾸미볶음","엽기떡볶이","매운갈비찜","불닭","매운족발","매운돼지갈비","불짬뽕","로제떡볶이","치즈떡볶이","매운탕","아귀찜","해물찜"] },
...
/** 카테고리 → 실제로 검색에 쓸 말들.
 *
 *  홈 화면의 카테고리(고기·국물…)와 모임 화면의 메뉴 분류(고기류·안주류…)를 함께
 *  담는다. 실측(네이버 묶음 검색, 강남역): 분류 이름 8개를 그대로 검색하면 총 6곳,
 *  아래 대체 검색어 8개로는 36곳이었다. 특히 고기류·한식디저트는 0곳이다. */
